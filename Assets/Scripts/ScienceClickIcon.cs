...
public class ScienceClickIcon : MonoBehaviour
{
    int Source;
    int Destination;
    float progress;
    ...
    void Start()
    {
        ...
        Vector3 prod = GameObject.Find("ScienceText").GetComponent<RectTransform>().position;
        //Vector3 prod = transform.TransformPoint(GameObject.Find("ProductivityValue").GetComponent<RectTransform>().position);
        destination = new Vector2(prod.x, prod.y);
    }
